...
from amp_stories.templates import (
    ...
    CARD_OVERLAY_LAYOUT,
    CENTER_FOCUS_LAYOUT,
    COMMERCE_CRISP_MOTION,
    EDITORIAL_SOFT_MOTION,
    SPLIT_PANEL_LAYOUT,
    TOP_STACK_LAYOUT,
    ChartRow,
    LayoutPreset,
...
class TestBackgroundLayers:
    ...
    def test_with_image_motion_panning_uses_story_panning_media(self) -> None:
        layers = _background_layers(
            "img.jpg",
            SLATE_THEME,
            motion=MotionPreset("pan-bg", background_media="panning"),
        )
        assert isinstance(layers[0].children[0], StoryPanningMedia)

    def test_with_image_overlay_false_returns_single_layer(self) -> None:
        layers = _background_layers("img.jpg", SLATE_THEME, overlay=False)
        assert len(layers) == 1


class TestTemplatePresets:
    def test_motion_preset_dataclass(self) -> None:
        preset = MotionPreset("custom", heading_animate_in="fade-in")
        assert preset.name == "custom"
        assert preset.heading_animate_in == "fade-in"

    def test_layout_preset_dataclass(self) -> None:
        preset = LayoutPreset("custom", wrapper_class="ast-panel")
        assert preset.name == "custom"
        assert preset.wrapper_class == "ast-panel"

    def test_builtin_layout_names(self) -> None:
        assert BOTTOM_STACK_LAYOUT.name == "bottom-stack"
        assert TOP_STACK_LAYOUT.name == "top-stack"
        assert CENTER_FOCUS_LAYOUT.name == "center-focus"
        assert CAPTION_BAND_LAYOUT.name == "caption-band"
        assert CARD_OVERLAY_LAYOUT.name == "card-overlay"
        assert SPLIT_PANEL_LAYOUT.name == "split-panel"

    def test_builtin_motion_names(self) -> None:
        assert EDITORIAL_SOFT_MOTION.name == "editorial-soft"
        assert ADVENTURE_CINEMATIC_MOTION.name == "adventure-cinematic"
        assert COMMERCE_CRISP_MOTION.name == "commerce-crisp"

    def test_motion_override_applies_heading_duration_and_delay(self) -> None:
        motion = MotionPreset(
            "custom",
            heading_animate_in="fade-in",
            animate_in_duration="0.2s",
            animate_in_delay="0.1s",
        )
        p = title_page("p1", "Title", eyebrow="Label", motion=motion)
        title_el = next(
            c
            for c in p.layers[-1].children
            if isinstance(c, TextElement) and c.class_ == "ast-title"
        )
        assert title_el.animate_in == "fade-in"
        assert title_el.animate_in_duration == "0.2s"
        assert title_el.animate_in_delay == "0.1s"

    def test_motion_override_applies_body_animation(self) -> None:
        motion = MotionPreset("custom", body_animate_in="fly-in-top")
        p = quote_page("q1", "Quote", attribution="Source", motion=motion)
        attr = next(
            c
            for c in p.layers[-1].children
            if isinstance(c, TextElement) and c.class_ == "ast-attribution"
        )
        assert attr.animate_in == "fly-in-top"
...
class TestTitlePage:
    ...
    def test_title_element_present(self) -> None:
        ...
        titles = [
            c for c in text_layer.children if isinstance(c, TextElement) and c.class_ == "ast-title"
        ]
        assert len(titles) == 1
    ...
    def test_title_is_h1(self) -> None:
        p = title_page("p", "Title")
        text_layer = p.layers[-1]
        h1s = [c for c in text_layer.children if isinstance(c, TextElement) and c.tag == "h1"]
        assert len(h1s) == 1
    ...
    def test_title_no_animation_when_none(self) -> None:
        theme = Theme(
            heading_animate_in=None,
            body_animate_in=None,
            bg_color="#000000",
            text_color="#ffffff",
            accent_color="#ff0000",
            muted_color="#888888",
...
class TestQuotePage:
    ...
    def test_quote_text_present(self) -> None:
        ...
        text_layer = p.layers[-1]
        bodies = [
            c for c in text_layer.children if isinstance(c, TextElement) and c.class_ == "ast-body"
        ]
        assert len(bodies) == 1
    ...
    def test_quote_uses_blockquote_tag(self) -> None:
        ...
        text_layer = p.layers[-1]
        blockquotes = [
            c for c in text_layer.children if isinstance(c, TextElement) and c.tag == "blockquote"
        ]
...
class TestTextPage:
    ...
    def test_heading_is_h2(self) -> None:
        p = text_page("t", "Heading", "Body")
        text_layer = p.layers[-1]
        h2s = [c for c in text_layer.children if isinstance(c, TextElement) and c.tag == "h2"]
        assert len(h2s) == 1
...
class TestCtaPage:
    ...
    def test_with_image_three_layers(self) -> None:
        p = cta_page("cta", "Heading", cta_url="https://example.com", background_src="img.jpg")
        assert len(p.layers) == 3
    ...
    def test_body_added_when_provided(self) -> None:
        p = cta_page("cta", "Heading", body="Some details here.", cta_url="https://example.com")
        text_layer = p.layers[-1]
        bodies = [
            c for c in text_layer.children if isinstance(c, TextElement) and c.class_ == "ast-body"
        ]
    ...
    def test_body_has_animate_in_delay(self) -> None:
        p = cta_page("cta", "Heading", body="Body text.", cta_url="https://example.com")
        text_layer = p.layers[-1]
        body_el = next(
            c for c in text_layer.children if isinstance(c, TextElement) and c.class_ == "ast-body"
        )
    ...
    def test_outlink_custom_cta_text(self) -> None:
        p = cta_page("cta", "Heading", cta_text="Visit site", cta_url="https://example.com")
        assert p.outlink is not None
    ...
    def test_outlink_custom_theme_accent(self) -> None:
        theme = Theme(
            bg_color="#000000", text_color="#ffffff", accent_color="#ff6600", muted_color="#aaaaaa"
        )
    ...
    def test_auto_advance_after(self) -> None:
        p = cta_page("cta", "Heading", cta_url="https://example.com", auto_advance_after="10s")
...
class TestIntegration:
    def test_themed_story_renders(self) -> None:
        pages = [
            title_page("p1", "Story Title", subtitle="Subtitle", background_src="hero.jpg"),
...
class TestVideoPage:
    ...
    def test_eyebrow_before_caption_same_layer(self) -> None:
        p = video_page("v1", "https://example.com/v.mp4", eyebrow="CAT", caption="A caption")
        assert len(p.layers) == 2
        text_layer = p.layers[1]
        assert text_layer.children[0].class_ == "ast-eyebrow"  # type: ignore[union-attr]
        assert text_layer.children[1].class_ == "ast-caption"  # type: ignore[union-attr]
...
class TestComparisonPage:
    ...
    def _get_row(self, p: Page) -> DivElement:
        ...
        rows = [
            ...
            if isinstance(c, DivElement) and c.class_ == "ast-comparison-row"
        ]
        assert len(rows) == 1
        return rows[0]
    ...
    def test_row_has_two_comparison_cols(self) -> None:
        ...
        cols = [
            ...
            if isinstance(c, DivElement) and c.class_ == "ast-comparison-col"
        ]
        assert len(cols) == 2
    ...
    def test_versus_shown_in_centre_div(self) -> None:
        ...
        vs_divs = [
            c for c in row.children if isinstance(c, DivElement) and c.class_ == "ast-comparison-vs"
        ]
    ...
    def test_eyebrow_in_vertical_layer(self) -> None:
        ...
        layer = self._get_vertical_layer(p)
        eyebrow_texts = [
            c for c in layer.children if isinstance(c, TextElement) and c.class_ == "ast-eyebrow"
        ]
    ...
    def test_stat_uses_comparison_stat_class(self) -> None:
        ...
        stat_els = [
            ...
            if isinstance(c, TextElement) and c.class_ == "ast-comparison-stat"
        ]
    ...
    def test_with_background_has_image_layer(self) -> None:
        ...
        has_img = any(isinstance(c, AmpImg) for lyr in fill_layers for c in lyr.children)
        assert has_img
...
class TestBreakingPage:
    ...
    def test_default_badge_is_breaking(self) -> None:
        ...
        text_layer = p.layers[-1]
        badges = [
            c for c in text_layer.children if isinstance(c, TextElement) and c.class_ == "ast-badge"
        ]
        assert len(badges) == 1
    ...
    def test_renderable(self) -> None:
        p = breaking_page("b1", "Market crash begins", badge="ALERT", body="Stocks fell 5% today.")
        story = _renderable_story([p])
        html = story.render()
...
class TestUpdatePage:
    ...
    def test_with_background_three_layers(self) -> None:
        p = update_page("u1", 1, "Headline", "Body", background_src="https://example.com/img.jpg")
        assert len(p.layers) == 3
...
class TestItineraryPage:
    ...
    def test_with_background_three_layers(self) -> None:
        p = itinerary_page("it1", 1, "Oslo", background_src="https://example.com/oslo.jpg")
        assert len(p.layers) == 3

    def test_renderable(self) -> None:
        p = itinerary_page("it1", 4, "Barcelona", details=["Sagrada Família", "La Barceloneta"])
        story = _renderable_story([p])
        html = story.render()
...
class TestDataChartPage:
    ...
    def test_with_background_three_layers(self) -> None:
        p = data_chart_page(
            "dc1", "Chart", [ChartRow("A", 1)], background_src="https://example.com/bg.jpg"
        )
        assert len(p.layers) == 3

    def test_renderable(self) -> None:
        rows = [ChartRow("Python", 45, display="45%"), ChartRow("JavaScript", 35, display="35%")]
...
class TestDealPage:
    ...
    def test_all_optional_fields_present(self) -> None:
        p = deal_page(
            "d1", "Title", badge="SALE", description="Desc", was_price="$100", price="$50"
        )
        text_layer = p.layers[-1]
...
# ---------------------------------------------------------------------------
# New template presets / page types
# ---------------------------------------------------------------------------


class TestTimelineStepPage:
    def test_returns_page(self) -> None:
        p = timeline_step_page("t1", "6:00 AM", "Storm arrives")
        assert isinstance(p, Page)

    def test_label_headline_body_present(self) -> None:
        p = timeline_step_page("t1", "6:00 AM", "Storm arrives", body="Winds intensify.")
        text_layer = p.layers[-1]
        texts = [c.text for c in text_layer.children if isinstance(c, TextElement)]
        assert texts == ["6:00 AM", "Storm arrives", "Winds intensify."]


class TestFactCheckPage:
    def test_uses_card_wrapper(self) -> None:
        p = fact_check_page("f1", "Claim text", "False")
        wrapper = p.layers[-1].children[0]
        assert isinstance(wrapper, DivElement)
        assert wrapper.class_ == "ast-panel ast-panel--card ast-measure"

    def test_renders_fact_check_content(self) -> None:
        p = fact_check_page("f1", "Claim text", "False", explanation="Source disagrees.")
        wrapper = p.layers[-1].children[0]
        assert isinstance(wrapper, DivElement)
        texts = [c.text for c in wrapper.children if isinstance(c, TextElement)]
        assert texts == ["FACT CHECK", "Claim text", "False", "Source disagrees."]


class TestKeyTakeawaysPage:
    def test_empty_takeaways_raises(self) -> None:
        with pytest.raises(ValidationError):
            key_takeaways_page("k1", "Takeaways", [])

    def test_bullets_present(self) -> None:
        p = key_takeaways_page("k1", "Takeaways", ["First", "Second"])
        wrapper = p.layers[-1].children[0]
        assert isinstance(wrapper, DivElement)
        bullet_texts = [
            c.text
...
class TestProcessStepPage:
    def test_step_title_body_present(self) -> None:
        p = process_step_page("p1", "STEP 1", "Collect data", "Use verified sources.")
        wrapper = p.layers[-1].children[0]
        assert isinstance(wrapper, DivElement)
        texts = [c.text for c in wrapper.children if isinstance(c, TextElement)]
        assert texts == ["STEP 1", "Collect data", "Use verified sources."]


class TestHeroVideoPage:
    def test_has_video_overlay_and_text_layers(self) -> None:
        p = hero_video_page("hv1", "https://example.com/v.mp4", "Headline", eyebrow="Watch")
        assert len(p.layers) == 3
        assert isinstance(p.layers[0].children[0], AmpVideo)
        assert isinstance(p.layers[1].children[0], DivElement)
        assert p.layers[1].children[0].class_ == "ast-overlay"
    ...
    def test_text_content_present(self) -> None:
        p = hero_video_page("hv1", "https://example.com/v.mp4", "Headline", subtitle="Sub")
        text_layer = p.layers[-1]
        texts = [c.text for c in text_layer.children if isinstance(c, TextElement)]
        assert texts == ["Headline", "Sub"]


class TestCardOverlayPage:
    def test_wraps_text_in_card_panel(self) -> None:
        p = card_overlay_page("c1", "Headline", body="Body copy", eyebrow="Intro")
        wrapper = p.layers[-1].children[0]
        assert isinstance(wrapper, DivElement)
        assert wrapper.class_ == "ast-panel ast-panel--card ast-measure"

    def test_renderable(self) -> None:
        p = card_overlay_page("c1", "Headline", body="Body copy", background_src="img.jpg")
        story = _renderable_story([p])
        html = story.render()
